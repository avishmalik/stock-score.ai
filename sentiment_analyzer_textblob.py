#!/usr/bin/env python3
"""
Sentiment Analyzer - Algorithm 1: TextBlob
Uses TextBlob library for sentiment analysis based on pattern analysis and word polarity.
"""
...
# Common companies (same as main analyzer)
COMMON_COMPANIES = {
...
def analyze_company_sentiment(company: str, contexts: List[str]) -> Dict:
    """Analyze sentiment for a specific company using TextBlob."""
    if not contexts:
        return {
            'company': company,
            'sentiment_score': 0.0,
...
def create_performance_chart(results: Dict, output_path: str):
    ...
    plt.title('Stock Performance Prediction - TextBlob Algorithm', fontsize=14, fontweight='bold')
...
def analyze_transcriptions(
    ...
    """Analyze all transcribed text files using TextBlob."""
    ...
    output = {
        ...
        'algorithm': 'TextBlob',
        'algorithm_description': 'Pattern analysis and word polarity using TextBlob library',
    ...
    json_path = os.path.join(output_dir, 'sentiment_textblob_predictions.json')
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(output, f, indent=2, ensure_ascii=False)
    
    print(f"✓ TextBlob predictions saved to: {json_path}")
    
    if create_chart:
        chart_path = os.path.join(output_dir, 'sentiment_textblob_chart.png')
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='Sentiment Analyzer - TextBlob Algorithm')
